...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.action === 'onLoading') {
        console.log("onLoading ...");        
        chrome.runtime.sendMessage({"is_recording":true}, function(is_recording){
            if(is_recording != true){
                // when recording is not running then recording falg is checking to start it
                const queryParams = new URLSearchParams(window.location.search);
                if (queryParams.get('recording') === 'true') {
                    console.log(true);
                    let textt='Start Recording';
                    chrome.runtime.sendMessage({"start_stop_button": textt},function(){ 
...
function recordingStickyButton() {
    ...
    button.style.animation = 'blinking 1s infinite';
    // Add animation keyframes
    const styleSheet = document.styleSheets[0];
    // styleSheet.insertRule('@keyframes blinking { 0%, 49.9%, 100% { opacity: 1; } 50%, 99.9% { opacity: 0; } }', styleSheet.cssRules.length);
    button.addEventListener('click', () => {
        let textt = 'Stop Recording';
        chrome.runtime.sendMessage({ "start_stop_button": textt }, function () { console.log("recording stopped successfully..."); });
        // end click
        button.remove();
    });
}
